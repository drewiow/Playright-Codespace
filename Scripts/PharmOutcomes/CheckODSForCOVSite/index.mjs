...
export default async function run({ logger = defaultLogger } = {}) {
    ...
    try {
        ...
        const inputCsvPath =
            ...
            path.resolve(__dirname, "input", "odsCodes.csv");
        ...
        try {
            ...
            odsCodes = parseCSV(raw);
            logger(`✅ Loaded ${odsCodes.length} odsCodes from CSV`);
        } catch (err) {
            logger("❌ Error reading odsCodes.csv:", err.message);
            throw err;
        }
        ...
        // Prepare output CSV
        const resultsCsvPath = path.join(runDir, "ODSCodes.csv");
        if (!fs.existsSync(resultsCsvPath)) {
            fs.writeFileSync(resultsCsvPath, "OdsCode,Region\n", "utf-8");
        }

        await logAction("Navigate to Outcomes4Health", () =>
            page.goto("https://outcomes4health.org/o4h/"),
            logger
        );

        await logAction("Click COVID", () =>
            page.getByText("COVID", { exact: true }).click(),
            logger
        );

        for (const row of odsCodes) {
            const odsCode = row.odsCode ?? "";
            logger(`\n👤 Processing ODS Code: ${odsCode}`);

            await page.fill("#o4hODSCode", odsCode + "COV");
            await page.click("#o4hCheckODSCode");

            const button = page.locator('button.o4hODSConfirm');

            let instance = "Primary";

            try {
                await button.waitFor({ state: "visible", timeout: 5000 });
                instance = await button.getAttribute("data-instance");
                logger(`Instance found: ${instance}`);
            } catch {
                logger("Button not found within timeout");
            }

            // Log to CSV
            const csvRow = `"${odsCode}","${instance}"\n`;
