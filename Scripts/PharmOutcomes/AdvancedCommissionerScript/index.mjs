...
import { login, editCommissioner, setupContext, parseCSV } from "../helpers.mjs";
...
export default async function run({ logger = defaultLogger } = {}) {
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        await editCommissioner(page, odsCode);
      } catch (err) {
        logger(`❌ editCommissioner failed for ${odsCode}: ${err?.message}`);
        continue;
      }
      /*
             Guard example
